...
int CMysqlClient::Connect(const KeyValue& args)
{
	...
	ret = mysql_real_connect(&m_db, args.at("host"), args.at("user"), args.at("password"), args.at("db"), atoi(args.at("port")), nullptr, 0);
...
_mysql_table_::_mysql_table_(const _mysql_table_& table)
{
	...
	for(size_t i = 0; i < table.FieldDefine.size(); i++){
		PField field = std::make_shared<_mysql_field_>(*
			dynamic_cast<_mysql_field_*>(table.FieldDefine[i].get()));
		FieldDefine.push_back(field);
		Fields[field->Name] = field;
	}
}

_mysql_table_::~_mysql_table_()= default;
...
{	//CREATE TABLE IF NOT EXISTS 表全名 (列定义,..., PRIMARY KEY `主键列名` ,UNIQUE INDEX `列名_UNIQUE` (列名 ASC) VISIBLE );
	...
	for(unsigned i = 0; i < FieldDefine.size(); i++){
		...
		if(FieldDefine[i]->Attr & UNIQUE){
			...
			sql += static_cast<Buffer>(*FieldDefine[i]) + " ASC) VISIBLE ";
		}
	}
	sql += ");";
	return sql;
...
{
	return "DROP TABLE" + static_cast<Buffer>(*this);
}
...
Buffer _mysql_field_::Create()
{
	...
	if((Attr & DEFAULT) && (!Default.empty()) && (Type != "BLOB") && (Type != "TEXT") && (Type != "GEOMETRY") && (Type != "JSON")){
		sql += " DEFAULT \"" + Default + "\" ";
	}
